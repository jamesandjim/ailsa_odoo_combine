...
class sale_subscription(http.Controller):
    ...
    @http.route(['/my/subscription/json_payment/<int:account_id>/',
                 '/my/subscription/json_payment/<int:account_id>/<string:uuid>'], type='json', auth="public")
    def payment_json(self, account_id, uuid=None, **kw):
        ...
        # if no payment has been selected for this account, then we display redirect to /my/subscription with an error message
        if len(account.payment_token_id) == 0:
            raise UserError(_('No payment method set on subscription'))
        ...
        return {
            'tx_info': tx_info,
            'redirect': '/my/subscription/%s/%s' % (account.id, account.uuid),
        }
